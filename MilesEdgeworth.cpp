...
void MilesEdgeworth::createContextMenu()
{
    ...
    connect(actionWeb, &QAction::triggered, [=](bool checked) {
        if (type == MilesEdgeworth::WEBING) {
            ...
            specifyChangeGif(QString(":/gifs/special/thinkb%1.gif").arg(next_direct),
                MilesEdgeworth::WEB, next_direct);
                actionWeb->setDisabled(true);  // 唤醒时解除禁用
...
connect(actionShowProsBadge, &QAction::triggered, [=](bool checked) {
    ...
    // 在前置动画完成后执行原来的徽章动画序列
    QTimer::singleShot(2800, this, [=]() { // 前置动画需要3秒
        petMovie->stop();
    // 设置动画文件名和类型
        QString filename;
    if (language == 0) {
...
void MilesEdgeworth::runMove()
{
    ...
    {
        // 向右走
        delta = (QPointF(unit, 0) * scale * 0.14).toPoint();
        break;
    }
    case 1:
    {
    ...
    {
        // 向右上走
        delta = (QPointF(unit, -unit) * scale * 0.14).toPoint();
        break;
    }
    case 3:
    {
    ...
    {
        // 向上走
        delta = (QPointF(0, -unit) * scale * 0.14).toPoint();
        break;
    }
    case 7:
    {
...
void MilesEdgeworth::walkMove()
{
    ...
    {
        // 向右走
        delta = (QPointF(unit, 0) * scale * 0.1).toPoint();
        break;
    }
    case 1:
    {
    ...
    {
        // 向右上走
        delta = (QPointF(unit, -unit) * scale * 0.1).toPoint();
        break;
    }
    case 3:
    {
    ...
    {
        // 向上走
        delta = (QPointF(0, -unit) * scale * 0.1).toPoint();
        break;
    }
    case 7:
    {
    ...
    QString filename;
     ...
     if (type == MilesEdgeworth::SLEEPING) {
        ...
        case FolderOpener::WindowState::WindowOpened:
            petMovie->stop();
            specifyChangeGif(QString(":/gifs/special/think%1.gif").arg(direction % 2),
